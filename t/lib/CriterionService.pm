...
sub getCriteria : Test(no_plan)
{
    ...
    if ($self->{sandbox}) {
        ...
        my $adgroup_id = $self->_get_adgroup_id();
        my $criterion_ids = [
            $self->{_criterion_id_0},
            $self->{_criterion_id_1},
        ];
...
sub setCampaignNegativeCriteria : Test(no_plan)
{
    ...
    if ($self->{sandbox}) {
        ...
        my $ret = $self->{obj}->setCampaignNegativeCriteria(
            $campaign_id,
...
sub updateCriteria : Test(no_plan)
{
    ...
    if ($self->{sandbox}) {
        ...
        my $ret 
            = $self->{obj}->updateCriteria($criterion1, $criterion2);
